...
public class Simulador {
	...
	public short[] memory = new short[MEMORY_SIZE];
	public short[] registradores = new short[8];
	...
	public Instrucoes decodificar(short instrucaoCodificada){
		...
		//instruções para o formato I:
			...
			decodificada.registradorr = extract_bits(instrucaoCodificada, 10, 3);
			decodificada.imediato = extract_bits(instrucaoCodificada, 0, 10);
		}
		return decodificada; 	
	}

	public boolean executar(Instrucoes instrucoes){
		if (instrucoes.format == 0){
			if(instrucoes.opcode == 0){ //add
				...
				registradores[instrucoes.destino] = memory[endereco];
				 //armazena no registrador um valor da memória

			}
			else if(instrucoes.opcode == 16){ //store
				memory[registradores[instrucoes.operando1]] = registradores[instrucoes.destino];
				//armazena na memoria um valor de um registrador
			}

			else if(instrucoes.opcode == 63){ //syscall para fechar
		...
		else if(instrucoes.format == 1){
			if(instrucoes.opcode == 3){
				registradores[instrucoes.registradorr] = instrucoes.imediato;
				//System.out.println(registradores[instrucoes.registradorr]);
				//System.out.println(instrucoes.registradorr);
			}
		}
		return true;
	}
